...
Simulation* simulation_instance[NUM_INSTANCES];
Queue* simulation_queues[NUM_INSTANCES];

Payload* new_payload(int seq_nr, double* y)
{
...
void initialize_simulation(double ts, double arrival_rate) {

	int i;
	for (i=0; i<NUM_INSTANCES; i++) {
		simulation_instance[i] = simulation_new(i, ts, arrival_rate);
		simulation_queues[i] = create_queue();
	}

	// 0: cart position, 1: cart velocity, 2: angle, 3: angular velocity
	double arr[] = {0, 0, 0.610865, 0};

	preset_pendulum(simulation_instance[0], arr, 4);
	
	if (DEBUG) {
		printf("DPDK: Simulation initialized - Using %d Instances\n", NUM_INSTANCES);
	}
}
