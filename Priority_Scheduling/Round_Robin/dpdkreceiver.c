...
int receive_pkt_ts(__attribute__((unused)) void *args) {
	...
	while(RCV_PKT) {
		...
		if(ret > 0) {
			...
			curr_ncs = rte_be_to_cpu_32(ipv4_header->src_addr) & 0xFF;
			prio = NUM_USED[curr_ncs];
			
			//printf("%d.%d.%d.%d\n", ((rte_be_to_cpu_32(ipv4_header->src_addr) >> 24) & 0xFF), ((rte_be_to_cpu_32(ipv4_header->src_addr) >> 16) & 0xFF),((rte_be_to_cpu_32(ipv4_header->src_addr) >> 8) & 0xFF),((rte_be_to_cpu_32(ipv4_header->src_addr) >> 0) & 0xFF));

			printf("Enqueuing pkt of %d with prio %d\n", curr_ncs, prio);
			
			Message* package = new_message(recd_bufs[0], curr_ncs);
